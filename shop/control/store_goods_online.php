...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_goodsOp() {
        ...
        // Jay
        //$goodscommon_info['areainfo'] = trim($model_area->find($goodscommon_info['areaid_1'])['area_name'].' '.$model_area->find($goodscommon_info['areaid_2'])['area_name']);
        // Jay_END
    ...
    public function edit_save_goodsOp() {
		...
		$update_common['sale_area_id']         = intval($_POST['sale_area_id']);
        ...
        if (is_array ( $_POST ['spec'] )) {
            ...
            if (!empty($goods_info)) {
				...
				$update['sale_area_id']        = $update_common['sale_area_id'];
                $update['goods_jingle']      = $update_common['goods_jingle'];
                $update['store_id']          = $_SESSION['store_id'];
                $update['store_name']        = $_SESSION['store_name'];
                $update['gc_id']             = $update_common['gc_id'];
	...
	public function ajaxOp(){
		switch ($_GET['branch']){
			...
			case 'set_price':
			    ...
			    $where = array('goods_commonid' => intval($_GET['id']));
				$update_array = array();
				$update_array['goods_price'] = $_GET['value'];
				$update_array['goods_marketprice'] = $_GET['value'];
				$update_array['goods_promotion_price'] = $_GET['value'];
				$update_array['goods_marketprice'] = sprintf("%.2f", $_GET['value']*1.46);
				$update_array2 = array();
				$update_array2['goods_price'] = $_GET['value'];
				$update_array2['goods_marketprice'] = sprintf("%.2f", $_GET['value']*1.46);
				$model_class->editGoods($update_array, $where);
				$model_class->editGoodsCommon($update_array2, $where);
			 ...
			 * 更新商品货号
			 */
			case 'set_goods_serial':
				$model_class = Model('goods');
			    $where = array('goods_commonid' => intval($_GET['id']));
				$update_array = array();
				$update_array['goods_serial'] = $_GET['value'];
				$model_class->editGoods($update_array, $where);
				$model_class->editGoodsCommon($update_array, $where);
				echo 'true';exit;
				break;
			/**
			 * 更新运费
			...
			case 'set_shipping':
			    ...
			    $where = array('goods_commonid' => intval($_GET['id']));
				$update_array = array();
				$update_array['goods_freight'] = $_GET['value'];
				$model_class->editGoods($update_array, $where);
				$model_class->editGoodsCommon($update_array, $where);
				echo 'true';exit;
				break;
