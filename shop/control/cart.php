...
class cartControl extends BaseBuyControl {
	...
	public function indexOp() {
        ...
        foreach ($cart_list as $cart) {
			...
			$rate = $model_goods->getGoodstaxrate($cart['gc_id']);
			$cart['tax'] = ncPriceFormat($rate['tax_rate'] * $cart['goods_total']);
	...
	public function updateOp() {
		...
		if ($cart_info['bl_id'] == '0') {
		    ...
		    if(intval($goods_info['goods_storage']) < $quantity) {
				...
				$rate = $model_goods->getGoodstaxrate($goods_info['gc_id']);
				$return['tax'] = $goods_info['goods_price']*$rate['tax_rate']*$quantity;
