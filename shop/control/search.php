...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        // 地区
        // Jay: get cityProvince data
        $province_array = Model('area')->getAreaNames();
        Tpl::output('province_array', $province_array);
		Tpl::output('cityProvince_array', $cityProvince_array);
...
//             Log::record('search\auto_complete'.$e->getMessage(),Log::RUN);
	...
	public function sale_areaOp(){
        ...
        if (is_array($indexer_ids)) {
			...
			if (intval($_GET['salearea_id']) > 0) {
                $condition['sale_area_id'] = intval($_GET['salearea_id']);
            }
            if ($_GET['type'] == 1) {
                $condition['is_own_shop'] = 1;
            }
