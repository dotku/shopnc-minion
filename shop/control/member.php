...
class memberControl extends BaseMemberControl{
	...
	/**
		...
		//订单商品
		$ordergoods= $ordergoods_model->getOnegoods($order_id,$fields = 'goods_id');
		*/
		$goods = Model('goods');
		
		$goods_xiaopiao = $goods->getGoodsDetail($goods_id);
		/**
		 * 上传购物小票
		 */
		if ($_POST['form_submit']==ok){
        if (!empty($_FILES['xiaopiao']['name'])){
				$upload = new UploadFile();
				$upload->set('default_dir','xiaopiao');
				$result = $upload->upfile('xiaopiao');
				if ($result){
					$_POST['xiaopiao'] = $upload->file_name;
					//删除旧认证图片
						if (!empty($_POST["old_xiaopiao"])){
							@unlink(BASE_UPLOAD_PATH.DS.'xiaopiao'.DS.$_POST["old_xiaopiao"]);
						}
				}else {
					showMessage($upload->error,'','','error');
				}
			}
          if (empty($_POST["xiaopiao"])){
							$_POST["xiaopiao"] = $_POST["old_xiaopiao"];
						}
		
        $param = array();
        $param['goods_id'] = $_POST['goods_id'];
        $param['xiaopiao_image'] = $_POST["xiaopiao"];
        $xiaopiao = $goods->xiaopiaoUpdate($param);
		if (!$xiaopiao){
        ...
        Tpl::output('goods_xiaopiao',$goods_xiaopiao);
        Tpl::showpage('xiaopiao_form','null_layout');
      } 
}
