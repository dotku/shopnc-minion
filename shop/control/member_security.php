...
class member_securityControl extends BaseMemberControl {
    ...
    public function send_bind_emailOp() {
		...
		if(C('ucenter_status')) {
				...
				*/
				...
				$model_ucenter->userEdit(array('login_name'=>$_SESSION['member_name'],'','email'=>trim($_POST['email'])));
			}
    ...
    public function modify_pwdOp() {
		...
		if(C('ucenter_status')) {
				...
				*/
				$model_ucenter = Model('ucenter');
				$model_ucenter->userEdit(array('login_name'=>$_SESSION['member_name'],'','password'=>trim($_POST["password"])));
			}
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
