...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <h5><?php  if(!empty($GLOBALS['setting_config']['tax_status'])) {?>
		  <span style="padding-right: 20px; float: right;">根据中国海关对私人物品进口的规定，每个订单关税计算额，如果大于&nbsp;<font color="#ff0000"><?php echo $GLOBALS['setting_config']['limit_tax']; ?>&nbsp;元</font>，客户需要按关税计算额即时交纳关税，否则可能被罚没。</span>
		  <?php }else{?>查看购物车商品清单，增加减少商品数量，并勾选想要的商品进入下一步操作。 <?php } ?></h5>
  </div>
    ...
    <table class="ncc-table-style" nc_type="table_cart">
      <thead>
        <tr>
		  ...
		  <?php  if(!empty($GLOBALS['setting_config']['tax_status'])) {?>
		  <th class="w120">关税计算<?php echo '('.$lang['currency_zh'].')';?></th>
		   <?php } ?>
      ...
      <tbody>
        ...
        <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" nc_group="<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo $cart_info['state'] ? '' : 'item_disabled';?>">
          ...
          <td class="w120"><?php if ($cart_info['state']) {?>
            ...
            <?php }?></td>
			<?php  if(!empty($GLOBALS['setting_config']['tax_status'])) {?>
		  <td class="w120"><em id="item<?php echo $cart_info['cart_id']; ?>_taxrate" nc_type="eachGoodsTotalTax"><?php echo $cart_info['tax'];?></em></td>
		  <?php }?>
        ...
        <tr>
          <td class="tr" colspan="20"><div class="ncc-store-account">
			  ...
			  <?php  if(!empty($GLOBALS['setting_config']['tax_status'])) {?>
		       <dl>
                <dt>关税计算合计：</dt>
                <dd><em nc_type="eachStoreTotalTax"></em><?php echo $lang['currency_zh'];?></dd>
              </dl>
			  <?php }?>
            </div></td>
        </tr>
	  ...
	  <?php  if(!empty($GLOBALS['setting_config']['tax_status'])) {?>
	    <tr>
          <td colspan="20"><div class="ncc-all-account">关税计算合计<em id="cartTotalTax"><?php echo $output['cart_totals']; ?></em><?php echo $lang['currency_zh'];?></div></td>
        </tr>
		<?php }?>
      </tfoot>
    </table>
