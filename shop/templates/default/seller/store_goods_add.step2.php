...
<h3>页面导航</h3>
  <ul>
    ...
    <li><a id="demo5Btn" href="#demo5" class="demoBtn">收据信息</a></li>
...
<div class="item-publish">
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <!-- 商品物流信息 E -->
      <h3 id="demo5" nctype="virtual_null" <?php if ($output['goods']['is_virtual'] == 1) {?>style="display:none;"<?php }?>>收据信息</h3>
      <dl nctype="virtual_null" <?php if ($output['goods']['is_virtual'] == 1) {?>style="display:none;"<?php }?>>
        <dt>是否开增值税发票：</dt>
        <dd>
          <ul class="ncsc-form-radio-list">
	  ...
	  <dl>
        <dt><?php echo $lang['store_goods_index_sale_area'].$lang['nc_colon']?></dt>
        <dd>
			<?php if(!empty($output['salearea_list']) && is_array($output['salearea_list'])){ ?>
				<?php if(!empty($output['storeinfo']) && is_array($output['storeinfo'])){ ?>
					<input type="hidden" name="sale_area_id" value="<?php foreach($output['salearea_list'] as $k => $v){ ?><?php if($output['storeinfo']['sale_area_id'] ==  $v['sale_area_id']){ echo $v['sale_area_id']; }}?>" />
					<?php foreach($output['salearea_list'] as $k => $v){ ?>
					<?php if($output['storeinfo']['sale_area_id'] ==  $v['sale_area_id']){ echo $v['sale_area_name']; }?>
					<?php } ?>
				<?php }else{ ?>
					<input type="hidden" name="sale_area_id" value="<?php foreach($output['salearea_list'] as $k => $v){ ?><?php if($output['goods']['sale_area_id'] ==  $v['sale_area_id']){ echo $v['sale_area_id']; }?><?php } ?>" />
					<?php foreach($output['salearea_list'] as $k => $v){ ?>
					<?php if($output['goods']['sale_area_id'] ==  $v['sale_area_id']){ echo $v['sale_area_name']; }?>
					<?php } ?>
				<?php } ?>
              <?php } ?>
...
<!---
          <p id="region">
            <select name="sale_area_id" id="sale_area_id">
              <?php if(!empty($output['salearea_list']) && is_array($output['salearea_list'])){ ?>
			  <?php if(!empty($output['storeinfo']) && is_array($output['storeinfo'])){ ?>
              <?php foreach($output['salearea_list'] as $k => $v){ ?>
                <option <?php if($output['storeinfo']['sale_area_id'] ==  $v['sale_area_id']){ ?>selected="selected"<?php } ?> value="<?php echo  $v['sale_area_id']; ?>"><?php echo $v['sale_area_name']; ?></option>
                <?php } ?>
				<?php }else{ ?>
					<?php foreach($output['salearea_list'] as $k => $v){ ?>
						<option <?php if($output['goods']['sale_area_id'] ==  $v['sale_area_id']){ ?>selected="selected"<?php } ?> value="<?php echo  $v['sale_area_id']; ?>"><?php echo $v['sale_area_name']; ?></option>
					<?php } ?>
				<?php } ?>
              <?php } ?>
            </select>
