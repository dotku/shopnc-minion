...
<table class="ncsc-default-table">
  <thead>
    <tr nc_type="table_header">
	  ...
	  <th class="w100"><?php echo $lang['store_goods_index_goods_transfee_charge'];?></th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="tl"><dl class="goods-name">
          ...
          <dd><?php echo $lang['store_goods_index_goods_no'].$lang['nc_colon'];?><span title="点击编辑" fieldid="<?php echo $val['goods_commonid']; ?>" ajax_branch="set_goods_serial" datatype="required"  fieldname="goods_serial" nc_type="inline_edit" required="1" class="tooltip"><?php if($val['goods_serial']){echo $val['goods_serial'];}else{echo "点击添加货号";} ?></span></dd>
          <dd class="serve"> <span class="<?php if ($val['goods_commend'] == 1) { echo 'open';}?>" title="店铺推荐商品"><i class="commend">荐</i></span> <span class="<?php if ($val['mobile_body'] != '') { echo 'open';}?>" title="手机端商品详情"><i class="icon-tablet"></i></span> <span class="" title="商品页面二维码"><i class="icon-qrcode"></i>
			...
			<a href="javascript:void(0)" <?php if ($output['storage_array'][$val['goods_commonid']]['xiaopiao_image']){; ?>class="ncsc-btn-mini ncsc-btn-green" <?php }else{; ?>class="ncsc-btn-mini ncsc-btn-red" <?php }; ?> nc_type="dialog" dialog_title="上传购物小票" dialog_id="my_address_add"  uri="index.php?act=member&op=xiaopiao&goods_id=<?php echo $output['storage_array'][$val['goods_commonid']]['goods_id']; ?>" dialog_width="650" title="上传购物小票"><?php if ($output['storage_array'][$val['goods_commonid']]['xiaopiao_image']){; ?>查看小票 <?php }else{; ?>上传小票<?php }; ?></a>
            <?php if ($val['is_fcode'] ==1) {?>
	  ...
	  <td class="sort">¥<span title="点击编辑" fieldid="<?php echo $val['goods_commonid']; ?>" ajax_branch="set_price" datatype="required"  fieldname="goods_price" nc_type="inline_edit" required="1" class="tooltip"><?php echo $val['goods_price']; ?></span></td>
	  <td class="sort">¥<span title="点击编辑" fieldid="<?php echo $val['goods_commonid']; ?>" ajax_branch="set_shipping" datatype="number"  fieldname="goods_freight" nc_type="inline_edit" required="1" class="tooltip"><?php echo $val['goods_freight']; ?></span></td>
      <td class="sort"><span <?php if ($output['storage_array'][$val['goods_commonid']]['alarm']) { echo 'style="color:red;"';}?>><span title="点击编辑" fieldid="<?php echo $val['goods_commonid']; ?>" ajax_branch="set_storage" datatype="number"  fieldname="goods_storage" nc_type="inline_edit" required="1" class="tooltip"><?php echo $output['storage_array'][$val['goods_commonid']]['sum']; ?></span><?php echo $lang['piece']; ?></span></td>
