...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3>新增关税类目</h3>
      <ul class="tab-base">
          <li><a href="index.php?act=set_tax&op=set_tax" nctype="category"><span>关税管理</span></a></li>
		  <li><a href="JavaScript:void(0);" nctype="category" class="current"><span>添加新税号</span></a></li>
      </ul>
    </div>
  </div>
  ...
  <table class="table tb-type2">
      ...
      <tbody>
        <?php if(!empty($output['show_goods_class']) && is_array($output['show_goods_class'])){?>
          ...
          <td class="w25pre"><strong><?php echo $gc_list['gc_name'];?></strong>
          </td>
		...
		<?php foreach($gc_list2['class3'] as $key=>$gc_list3){?>
			  <?php if( $gc_list3['tax_number'] == '' or $gc_list3['tax_rate'] == '0'){?>
           <tr class="hover edit">
		   <td class="w48 sort"><span title="<?php echo $lang['nc_editable'];?>" ajax_branch="goods_class_sort" datatype="number" fieldid="<?php echo $gc_list3['gc_id'];?>" fieldname="gc_sort" nc_type="inline_edit" class="editable tooltip"><?php echo $output['gc_list'][$child]['gc_sort'];?></span></td>
