    ...
    <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
      ...
      <tbody>
        ...
        <tr>
		 ...
		 <tr>
		 <th>销售区域：</th>
          <td align="left"><select name="sale_area_id" id="sale_area_id">
              <?php if(!empty($output['salearea_list']) && is_array($output['salearea_list'])){ ?>
              <?php foreach($output['salearea_list'] as $k => $v){ ?>
				<option <?php if($output['joinin_detail']['sale_area_id'] ==  $v['sale_area_id']){ ?>selected="selected"<?php } ?> value="<?php echo  $v['sale_area_id']; ?>"><?php echo $v['sale_area_name']; ?></option>
                <?php } ?>
              <?php } ?>
            </select>
