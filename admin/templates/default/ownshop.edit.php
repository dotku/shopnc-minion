  ...
  <form id="store_form" method="post">
    ...
    <table class="table tb-type2">
      <tbody>
		...
		<tr>
          <td colspan="2" class="required"><label class="validation" for="member_passwd">销售区域：</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop"><select name="sale_area_id" id="sale_area_id">
              <?php if(!empty($output['salearea_list']) && is_array($output['salearea_list'])){ ?>
              <?php foreach($output['salearea_list'] as $k => $v){ ?>
                <option <?php if($output['store_array']['sale_area_id'] ==  $v['sale_area_id']){ ?>selected="selected"<?php } ?> value="<?php echo  $v['sale_area_id']; ?>"><?php echo $v['sale_area_name']; ?></option>
                <?php } ?>
              <?php } ?>
            </select></td>
          <td class="vatop tips"></td>
