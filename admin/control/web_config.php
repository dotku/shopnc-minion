...
class web_configControl extends SystemControl {
	 ...
	 * 商品分类
	 */
	public function category_list2Op() {
		$model_class = Model('goods_class');
	...
	public function homeview_apiOp() {
		$this->homeview_indexUpdateOp();
		/*
		...
		// 检查必填项
		$requiredFields = array('homeview_id', 'propertyName', 'propertyValue');
		/*
		if (!isset($_GET['homeview_id'])) {
			$result['msg'] = 'homeview_id ' . Language::get('required') . EOL;
		...
		if (!is_int(filter_input(INPUT_GET, "homeview_id", FILTER_VALIDATE_INT))){
			$result['msg'] = 'homeview_id ' . Language::get('must_be_int');
			echo json_encode($result, JSON_UNESCAPED_UNICODE);
			return;
		}
		echo "success";
		...
		if ($_GET['homeview_id']) {
			// 检查 homeview_id
			if (!is_int($_GET['homeview_id'])){
				$ifFound = $model_homeview->find($_GET['homeview_id']);
			} else {
				$result['update_status'] = 'homeview_id ' . Language::get('must_be_int');
			}

			if (isset($_GET['propertyName']) && isset($_GET['propertyValue']) && $ifFound) {
				$data['homeview_id'] = $_GET['homeview_id'];
				$data[$_GET['propertyName']] = intval($_GET['propertyValue']);
				$date['update_time'] = time();
				$model_homeview->update($data);
				$result['update_status'] = Language::get('update_success');
			} else {
				$result['update_status'] = Language::get('update_failed');
			}
	...
	public function web_htmlOp(){
		...
		if(!empty($web_array) && is_array($web_array)) {
			...
			showMessage(Language::get('nc_common_op_succ'),'index.php?act=web_config&op=code_edit&web_id=' . $web_id);
		} else {
			showMessage(Language::get('nc_common_op_fail'));
		}
	}
	...
	public function recycle_indexOp(){
		...
		Tpl::output('trash_list', $homeview_trash_list);
		Tpl::output('db_table', 'web_homeview'); // 目前只处理 web_homeview
		Tpl::showpage('web_config_recycle');
	}
}
