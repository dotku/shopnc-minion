...
class goods_class1Control extends SystemControl{
	private $links = array(
		array('url'=>'act=goods_class1&op=goods_class','lang'=>'nc_manage'),
		array('url'=>'act=goods_class1&op=goods_class_add','lang'=>'nc_new'),
		array('url'=>'act=goods_class1&op=goods_class_export','lang'=>'goods_class_index_export'),
		array('url'=>'act=goods_class1&op=goods_class_import','lang'=>'goods_class_index_import'),
		array('url'=>'act=goods_class1&op=tag','lang'=>'goods_class_index_tag'),
	);
	...
	public function goods_classOp(){
		...
		$model_class = Model('goods_class1');
		if (chksubmit()){
			//删除
		...
		if ($_GET['ajax'] == '1'){
			...
			Tpl::showpage('goods_class1.index');
		}
				...
				if ($result){
    				if ($insert_array['gc_parent_id'] == 0) {
            			if (!empty($_FILES['pic']['name'])) {//上传图片
                			...
                			$upload->set('file_name','newcategory-pic-'.$result.'.jpg');
            				$upload->upfile('pic');
            			}
    				}
					$url = array(
						array(
							'url'=>'index.php?act=goods_class1&op=goods_class_add&gc_parent_id='.$_POST['gc_parent_id'],
							'msg'=>$lang['goods_class_add_again'],
						),
						array(
							'url'=>'index.php?act=goods_class1&op=goods_class',
	...
	public function goods_class_editOp(){
		...
		if (chksubmit()){
			...
			if (!empty($_FILES['pic']['name'])) {//上传图片
				$upload = new UploadFile();
    			$upload->set('default_dir',ATTACH_COMMON);
    			$upload->set('file_name','newcategory-pic-'.$update_array['gc_id'].'.jpg');
				$upload->upfile('pic');
			}
			// 更新该分类下子分类的所有类型。
			...
			$url = array(
				array(
					'url'=>'index.php?act=goods_class1&op=goods_class_edit&gc_id='.intval($_POST['gc_id']),
		...
		Tpl::output('parent_list',$parent_list);
		// 一级分类列表
		$gc_list = H('goods_class1') ? H('goods_class1') : H('goods_class1', true);
		Tpl::output('gc_list', $gc_list);
		...
		Tpl::output('top_link',$this->sublink($this->links,'goods_class'));
		Tpl::showpage('goods_class.edit1');
	}
	...
	public function goods_class_importOp(){
		...
		if (chksubmit()){
			...
			if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
				...
				showMessage($lang['nc_common_op_succ'],'index.php?act=goods_class1&op=goods_class');
			}else {
	...
	public function goods_class_delOp(){
		...
		if (intval($_GET['gc_id']) > 0){
			...
			showMessage($lang['nc_common_del_succ'],'index.php?act=goods_class1&op=goods_class');
		}else {
			$this->log(L('nc_delete,goods_class_index_class') . '[ID:' . intval($_GET['gc_id']) . ']',0);
			showMessage($lang['nc_common_del_fail'],'index.php?act=goods_class1&op=goods_class');
		}
	}
	...
	public function tag_resetOp(){
		...
		if(!$return){
			showMessage($lang['goods_class_reset_tag_fail'], 'index.php?act=goods_class1&op=tag');
		}
		...
		if(is_array($goods_class) and !empty($goods_class)) {
			...
			if($return){
				...
				showMessage($lang['nc_common_op_succ'], 'index.php?act=goods_class1&op=tag');
			}else{
				$this->log(L('nc_reset').'tag',0);
				showMessage($lang['nc_common_op_fail'], 'index.php?act=goods_class1&op=tag');
			}
		}else{
			$this->log(L('nc_reset').'tag',0);
			showMessage($lang['goods_class_reset_tag_fail_no_class'], 'index.php?act=goods_class1&op=tag');
		}
	}
	...
	public function tag_updateOp(){
		...
		if(is_array($tag_list) && !empty($tag_list)){
			...
			$this->log(L('nc_update').'tag',0);
			showMessage($lang['goods_class_update_tag_fail_no_class'], 'index.php?act=goods_class1&op=tag');
		}
