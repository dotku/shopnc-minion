...
class memberControl extends SystemControl{
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				*/
				...
				$model_ucenter->userEdit(array('login_name'=>$_SESSION['member_name'],'','password'=>trim($_POST['member_passwd']),'','email'=>trim($_POST['member_email'])));
				}
				if ($result){
					$url = array(
					array(
	...
	public function ajaxOp(){
		switch ($_GET['branch']){
		 	...
		 	*/
			$model_ucenter = Model('ucenter');
			$result = $model_ucenter->checkUserExit(trim($_GET['member_name']));
			if($result == 1) {
				echo 'true';exit;
			} else {
				echo 'false';exit;
