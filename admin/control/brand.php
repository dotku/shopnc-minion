...
class brandControl extends SystemControl{
	...
	public function brandOp(){
		...
		$brand_list = $model_brand->getBrandList($condition, "*", 50);
	...
	public function brand_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$insert_array = array();
				$insert_array['brand_name'] = trim($_POST['brand_name']);//品牌名
				$insert_array['brand_initial'] = strtoupper($_POST['brand_initial']);//首字母
				$insert_array['class_id']	= $_POST['class_id'];//最后一个的分类id
				$insert_array['brand_class'] = trim($_POST['brand_class']);//最后一个的分类名
				$insert_array['brand_pic'] = trim($_POST['brand_pic']);
				$insert_array['brand_recommend'] = trim($_POST['brand_recommend']);//是否推荐
				$insert_array['brand_sort'] = intval($_POST['brand_sort']);//排序
    ...
    public function insertExcelData($data){
        ...
        foreach($data as $key=>$val){
            $arr=array();
            $arr['brand_name']=$val[1];
            $arr['brand_initial']=strtoupper($val[2]);
            $arr['brand_class']=$val[3];
            $arr['class_id']=$val[4];
            $arr['brand_pic']='';
            $arr['brand_sort']=(int)$val[5];
            $arr['brand_recommend']=(int)$val[6];
            $arr['store_id']=0;
            $arr['brand_apply']=1;
            $arr['show_type']=0;
            $insert_arr[]=$arr;
        }
        $result=$model_brand->addLotBrand($insert_arr);
        if($result>0){
            return true;
        }else{
            return false;
    ...
    public function brand_outglassOp(){

        $model_class = Model('goods_class');
        $sort_result=$model_class->getGoodsClassTree();
        foreach($sort_result as $key=>$val){
            foreach($val as $_k=>$_v){
                $sort_result[$key][$_k]= iconv("utf-8","gb2312//IGNORE",$_v);
            }
        }
        $new_sort=$this->getDataTree($sort_result);
    ...
    public function getDataTree($rows, $id='gc_id',$pid = 'gc_parent_id',$child = 'child',$root=0)
    {
        $tree = array();
        if(is_array($rows)){
            $array = array();
    ...
    public function hasChildArr($arr)
    {
       if(isset($arr['child'])) {
           return true;
       }else{
