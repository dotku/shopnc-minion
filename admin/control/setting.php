...
class settingControl extends SystemControl{
	...
	public function baseOp(){
		...
		if (chksubmit()){
			...
			$update_array['site_shares'] = trim($_POST['site_shares']);
			$update_array['limit_tax'] = trim($_POST['limit_tax']);
			$update_array['tax_status'] = trim($_POST['tax_status']);
			$update_array['reg_onoff'] = trim($_POST['reg_onoff']);
			$update_array['reg_close'] = trim($_POST['reg_close']);
	...
	public function sale_areaOp(){
		$lang = Language::getLangContent();
		$model_area = Model('setting');
		$model_sale_area = Model();
		if ($_POST['form_submit'] == 'ok'){
		$insert_array = array();
		$insert_array['sale_area_name'] = trim($_POST['sale_area_name']);
		$result = $model_area->saleareaadd($insert_array);
		if ($result){
        showMessage($lang['add_sale_area_ok'],$url);
					}else {
						showMessage($lang['add_sale_area_fail']);
					}};

		$sale_area = $model_sale_area->table('sale_area')->order('sale_area_id desc')->page(20)->select();
		Tpl::output('depot',$model_sale_area->showpage());
		Tpl::output('sale_area',$sale_area);
		Tpl::showpage('sale_area');
		
	}
	public function sale_area_delOp(){
		$lang	= Language::getLangContent();
		$model_area = Model('setting');
		if (intval($_GET['del_area_id']) > 0){
			$model_area->saleareadel(intval($_GET['del_area_id']));
			unset($depot_array);
			showMessage($lang['add_area_del_ok'],'index.php?act=setting&op=sale_area');
		}else {
			showMessage($lang['add_sarea_del_fail'],'index.php?act=setting&op=sale_area');
		}
	}
