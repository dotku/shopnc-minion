...
class typeControl extends SystemControl {
	...
	public function typeOp(){
		 ...
		 * 检索条件
		 */
		if (!empty($_GET['search_type_name'])){
			$condition['type_name'] = array('like',"%".$_GET['search_type_name']."%");
		}
		if (!empty($_GET['search_type_class'])){
			$condition['class_name'] = array('like',"%".$_GET['search_type_class']."%");
		}
/*
		$page	= new Page();
		$page->setEachNum(100);
		...
		*/
		$type_list = $model_type->sotypeList($condition, "*", 50);

		Tpl::output('type_list',$type_list);
		Tpl::output('search_type_name',trim($_GET['search_type_name']));
		Tpl::output('search_type_class',trim($_GET['search_type_class']));
		Tpl::output('page',$model_type->showpage());
