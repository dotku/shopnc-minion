...
function change_quantity(cart_id, input){
    ...
    $.getJSON('index.php?act=cart&op=update&cart_id=' + cart_id + '&quantity=' + _value, function(result){
    	...
    	if(result.state == 'true'){
			...
			$('#item' + cart_id + '_taxrate').html(number_format(result.tax,2));
            subtotal.html(number_format(result.subtotal,2));
...
function calc_cart_price() {
    ...
    obj.children('tbody').each(function(){
        //购物车每个店铺已选择商品关税的总价格
        var eachTotalTax = 0;
        $(this).find('em[nc_type="eachGoodsTotalTax"]').each(function(){
            if ($(this).parent().parent().find('input[type="checkbox"]').eq(0).attr('checked') != 'checked') return;
            eachTotalTax = eachTotalTax + parseFloat($(this).html());  
        });
        allTotalTax += eachTotalTax;
        $(this).children('tr').last().find('em[nc_type="eachStoreTotalTax"]').eq(0).html(number_format(eachTotalTax,2));
    });
    $('#cartTotalTax').html(number_format(allTotalTax,2));
}
$(function(){
    calc_cart_price();
