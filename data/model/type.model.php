...
class typeModel extends Model {
        ...
        if ($type_id > 0) {
            ...
            if (is_array($spec_list) && !empty($spec_list)) {
                ...
                foreach ($spec_list as $val) {
					...
					$spec_value_list = Model('spec')->getSpecValueList(array('sp_id'=>$val['sp_id'], 'gc_id'=>'0', 'store_id' => 0));
                   
                    foreach ($spec_value_list as $v) {
                        $c = array();
                        $c['sp_value_id'] = $v['sp_value_id'];
                        $c['sp_value_name'] = $v['sp_value_name'];
                        $c['sp_value_color'] = $v['sp_value_color'];
                        $a[] = $c;
                        $spec_json[$val['sp_id']][$v['sp_value_id']]['sp_value_name'] = $v['sp_value_name'];
                        $spec_json[$val['sp_id']][$v['sp_value_id']]['sp_value_color'] = $v['sp_value_color'];
                    }
	...
	public function sotypeList($condition, $field = '*', $page = 0, $order = 'type_sort asc, type_id desc', $limit = '') {
