...
class goodsModel extends Model{
	 ...
	 * by joe
     */
    public function getGoodstaxrate($gc_id, $field = '*') {
		$condition = array();
        $condition['gc_id'] = $gc_id;
        return $this->table('goods_class')->field($field)->where($condition)->find();
    }
    ...
    public function getGoodsDetail($goods_id) {
        ...
        $goods_info=$this->virtualSelnum($goods_info);
    ...
    public function virtualSelnum($goodsinfo){
        $todaystart=mktime(0,0,0,date('m'),date('d'),date(Y));
        $rand=mt_rand(1,10);
        $rand_start=mt_rand(20,200);
        $model=Model("goods_virtual");
        $time=time();
        $virtual=$model->where(array('goods_id'=>$goodsinfo['goods_id']))->find();
        if(empty($virtual)){

            $data=array(
                'goods_id'=>$goodsinfo['goods_id'],
                'selnum'=>$goodsinfo['goods_salenum']+$rand_start,
                'lastup'=>"{$time}",
            );
            $result=$model->insert($data);
            $goodsinfo['salenum_vir']=$data['selnum'];

        }else{

            if($virtual['lastup']<$todaystart){

                $data=array(
                    'goods_id'=>$goodsinfo['goods_id'],
                    'selnum'=>$virtual['selnum']+$rand,
                    'lastup'=>"{$time}",
                );
                $model->where(array("goods_id"=>$goodsinfo['goods_id']))->update($data);
                $goodsinfo['salenum_vir']=$data['selnum'];
            }else{

                $goodsinfo['salenum_vir']=$virtual['selnum'];
            }
        }
        return $goodsinfo;
    }
