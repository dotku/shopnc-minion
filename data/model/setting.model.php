...
class settingModel extends Model{
	...
	public function saleareaadd($param){
        if (empty($param)){
            return false;
        }
        if (is_array($param)){
            ...
            $result = Db::insert('sale_area',$tmp);
            return $result;
        }else {
            return false;
        }
    }
	public function saleareadel($id){
		if (intval($id) > 0){
			$where = " sale_area_id = '". intval($id) ."'";
			$result = Db::delete('sale_area',$where);
			return $result;
		}else {
			return false;
		}
	}
	public function getsaleareaList() {
        return $this->table('sale_area')->order('sale_area_id asc')->select();
    }
