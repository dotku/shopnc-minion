    ...
    public function calcCartList($store_cart_list) {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            ...
            foreach ($store_cart as $key => $cart_info) {
				...
				$rate = $model_goods->getGoodstaxrate($cart_info['gc_id']);
				$store_cart[$key]['rate'] = ncPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']*$rate['tax_rate']);
				$tmp_taxrate += $store_cart[$key]['rate'];
			...
			if ($tmp_taxrate <= $GLOBALS['setting_config']['limit_tax'] || (empty($GLOBALS['setting_config']['tax_status']))){
			           $tmp_taxrate=0;
			}
			$store_goods_total_tax[$store_id] = ncPriceFormat($tmp_taxrate);
        }
        return array($store_cart_list,$store_goods_total,$store_goods_total_tax);
    }
