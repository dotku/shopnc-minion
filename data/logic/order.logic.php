...
class orderLogic {
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '') {
        try {
			...
			//增加股份 by joe
			$Sharesdata = array();
			$Sharesdata['shares_sn'] = round($order_id*100+rand(1,100));
			$Sharesdata['order_sn'] = $order_info['order_sn'];
			$Sharesdata['store_id'] = $order_info['store_id'];
			$Sharesdata['order_amount'] =  $order_info['order_amount'];
			$Sharesdata['shares_amount'] = round($order_info['order_amount']*$GLOBALS['setting_config']['site_shares']/100);
			$Sharesdata['add_time'] = time();
			$Sharesdata['type'] = 0;

			Model('shares')->addShares($Sharesdata);
			Model('shares')->setShopShares($order_info['store_id'],$Sharesdata['shares_amount']);
